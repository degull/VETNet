# E:\restormer+volterra\Restormer + Volterra\test_unified.py
# Unified benchmark test script (PSNR/SSIM 출력 + (옵션) triplet PNG 저장)
# - Rain100H / Rain100L / GoPro / RESIDE-6K / CSD
# - AMP 지원
# - (옵션) tile inference 지원(OOM 방지)
#
# ✅ 네 프로젝트 구조 기준:
#   - models/restormer_volterra.py
#   - re_dataset/*.py (repo root에 있음)
#   - 이 스크립트는 "Restormer + Volterra" 폴더 안에서 실행
...
# ✅ 네 최종 pth로 교체해서 사용
CKPT_PATH = r"E:\restormer+volterra\checkpoints\restormer_volterra_mixed7\epoch_048_ssim0.9119_psnr30.14.pth"

# ✅ 평가 입력 해상도 (통일)
EVAL_SIZE = 256

# ✅ Tile inference (OOM 방지). 보통 0이면 비활성, 256~512 추천
USE_TILE = False
TILE_SIZE = 256
TILE_OVERLAP = 32
...
RESULTS_DIR = r"E:\restormer+volterra\results\unified_test"
...
def build_loader_for_dataset(name: str):
    ...
    if name == "Rain100H":
        ...
        # ✅ 네 GoProDataset 시그니처: GoProDataset(csv_file, transform=None)
...
def eval_one_dataset(model, name: str) -> Tuple[float, float]:
    ...
    if SAVE_TRIPLETS:
        print(f"  🖼️  Triplets saved: {min(saved, SAVE_N_PER_DATASET)} -> {save_dir}")
