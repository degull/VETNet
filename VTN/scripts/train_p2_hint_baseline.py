...
HINT_DIR = WORKSPACE_DIR / "baselines" / "HINT"

for p in [ROOT_DIR, HINT_DIR]:
...
def build_hint(model_size: str):
    if model_size == "S":
        return HINT(dim=32, num_blocks=[2, 3, 3, 4], num_refinement_blocks=2, heads=[1, 2, 4, 8])
    if model_size == "B":
        return HINT(dim=48, num_blocks=[4, 6, 6, 8], num_refinement_blocks=4, heads=[1, 2, 4, 8])
    raise ValueError("--model-size must be S or B")


def parse_args():
    parser = argparse.ArgumentParser(description="Train a fair P2 all-in-one HINT baseline.")
    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--start-epoch", type=int, default=1)
    parser.add_argument("--batch-size", type=int, default=2)
    parser.add_argument("--lr", type=float, default=2e-4)
    parser.add_argument("--max-per-task", type=int, default=160)
    parser.add_argument("--save-name", default="hint_b_p2_reside_all_in_one")
...
def main():
    ...
    if not HINT_DIR.exists():
        raise FileNotFoundError(f"HINT repo was not found: {HINT_DIR}")
    ...
    model = build_hint(args.model_size).to(device)
    if args.resume:
    ...
    print(f"[Protocol] P2 all-in-one HINT-{args.model_size} baseline.")
    print("[Protocol] Official HINT architecture, retrained from scratch.")
    ...
    with open(log_dir / "protocol.txt", "w", encoding="utf-8") as f:
        f.write(f"P2 all-in-one HINT-{args.model_size} baseline for Table 1\n")
