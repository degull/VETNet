...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate a P2 all-in-one AdaIR baseline.")
    parser.add_argument("--ckpt", required=True)
    parser.add_argument("--result-name", default="p2_adair_baseline")
    parser.add_argument("--table-out", default=None,
                        help="Optional CSV path for the Table 1 AdaIR row.")
...
def main():
    ...
    print("[Protocol] P2 all-in-one AdaIR baseline, official architecture retrained on same mixture as VTN.")
    ...
    print("\n==============================")
    print("P2 AdaIR Baseline Summary")
    ...
    table_out = Path(args.table_out) if args.table_out else ROOT_DIR / "experiments" / "table1_adair_row.csv"
    table_out.parent.mkdir(parents=True, exist_ok=True)
    row = {
        "Method": "AdaIR",
        "Status": "Retrained",
        "Direct Comparison?": "Yes",
        "Training Data": "Same as VTN" if args.max_items is None else f"Same as VTN; interim eval N={args.max_items}",
        ...
        "Protocol": f"P2 all-in-one, official AdaIR, same balanced160 protocol as VTN, max_items={args.max_items}",
    }
    ...
    with open(md_out, "w", encoding="utf-8") as f:
        ...
        f.write("|---|---|---|---|---:|---:|---:|---:|---:|\n")
        f.write(
            f"| AdaIR | Retrained | Yes | Same as VTN | "
