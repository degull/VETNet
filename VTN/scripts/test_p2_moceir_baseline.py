...
from train_p2_moceir_baseline import build_moceir


def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate a fair P2 all-in-one MoCE-IR baseline.")
    parser.add_argument("--ckpt", required=True)
    parser.add_argument("--model-size", choices=["S", "B"], default="S")
    parser.add_argument("--result-name", default="p2_moceir_baseline")
    parser.add_argument("--table-out", default=None,
                        help="Optional CSV path for the Table 1 MoCE-IR row.")
    parser.add_argument("--max-items", type=int, default=None,
                        help="Evaluate only the first N samples of each dataset for interim checks.")
    parser.add_argument("--save-triplets", action="store_true",
                        help="Save visual triplets during evaluation.")
...
def main():
    ...
    method_name = f"MoCE-IR-{args.model_size}"
    print(f"[Device] {device}")
    print(f"[CKPT ] {args.ckpt}")
    print(f"[Save ] RESULTS_DIR={results_dir}")
    print(f"[Protocol] P2 all-in-one {method_name} baseline, official architecture retrained on same mixture as VTN.")
    ...
    results = {}
    for name in names:
        if args.max_items is None and args.save_triplets:
    ...
    table_out = Path(args.table_out) if args.table_out else ROOT_DIR / "experiments" / "table1_moceir_row.csv"
    table_out.parent.mkdir(parents=True, exist_ok=True)
    training_data = "Same as VTN" if args.max_items is None else f"Same as VTN; interim eval N={args.max_items}"
    row = {
        "Method": method_name,
        "Status": "Retrained",
        ...
        "Protocol": f"P2 all-in-one, official {method_name}, same balanced160 protocol as VTN, max_items={args.max_items}",
    }
