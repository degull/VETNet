...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate a P2 all-in-one PromptIR baseline.")
    parser.add_argument("--ckpt", required=True)
    parser.add_argument("--result-name", default="p2_promptir_baseline")
    parser.add_argument("--table-out", default=None,
                        help="Optional CSV path for the Table 1 PromptIR row.")
...
def main():
    ...
    print("[Protocol] P2 all-in-one PromptIR baseline, official architecture retrained on same mixture as VTN.")
    ...
    print("\n==============================")
    print("P2 PromptIR Baseline Summary")
    ...
    table_out = Path(args.table_out) if args.table_out else ROOT_DIR / "experiments" / "table1_promptir_row.csv"
    ...
    row = {
        ...
        "Protocol": "P2 all-in-one, official PromptIR, same balanced160 protocol as VTN",
    }
    ...
    with open(md_out, "w", encoding="utf-8") as f:
        ...
        f.write("|---|---|---|---|---:|---:|---:|---:|---:|\n")
        f.write(
            f"| PromptIR | Retrained | Yes | Same as VTN | "
