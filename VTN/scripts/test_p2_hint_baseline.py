...
from train_p2_hint_baseline import build_hint


def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate a fair P2 all-in-one HINT baseline.")
    parser.add_argument("--ckpt", required=True)
    parser.add_argument("--model-size", choices=["S", "B"], default="B")
    parser.add_argument("--result-name", default="p2_hint_baseline")
...
def main():
    ...
    method_name = f"HINT-{args.model_size}"
    ...
    table_out = Path(args.table_out) if args.table_out else ROOT_DIR / "experiments" / "table1_hint_row.csv"
