...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, volterra_rank,
                 use_volterra_mdta=True, use_volterra_gdfn=True, volterra_order=2):
        super().__init__()
        ...
        self.volterra1 = VolterraLayer2D(dim, dim, rank=volterra_rank, order=volterra_order) if use_volterra_mdta else nn.Identity()
        ...
        self.volterra2 = VolterraLayer2D(dim, dim, rank=volterra_rank, order=volterra_order) if use_volterra_gdfn else nn.Identity()
