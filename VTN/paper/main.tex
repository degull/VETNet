...
To avoid ambiguity between training settings, we report four protocol groups separately. A result is directly comparable only within the same protocol group and exposure setting.

\paragraph{P1: Single-task training.}
...
\paragraph{P2: All-in-one training.}
A single shared model is trained jointly on multiple degradation types. This table is the primary evidence for unified restoration. Baselines are directly ranked against \ours only when they are retrained under the same training mixture or evaluated from a compatible official checkpoint. Results copied from prior papers or trained with non-identical data are marked as reported/non-identical and are used only as context.

\paragraph{P3: Zero-shot and cross-degradation evaluation.}
Models are evaluated on unseen degradation types or unseen task combinations without retraining. These results measure generalization and are not mixed with retrained all-in-one comparisons.

\paragraph{P4: Composite-degradation evaluation.}
Models are evaluated on images containing multiple degradations simultaneously. We report composite zero-shot and composite-trained results separately, since training exposure changes the interpretation of the comparison.
...
\begin{table*}[t]
\centering
\caption{Primary unified restoration comparison under the all-in-one protocol. ``Retrained'' means the method is trained using the same multi-task data mixture as \ours. ``Official-ckpt'' indicates evaluation from a compatible official checkpoint. ``Reported'' indicates numbers taken from the corresponding paper under a non-identical protocol and is not used for direct ranking.}
\label{tab:all_in_one}
\small
\begin{tabular}{lcccccccccc}
\toprule
\multirow{2}{*}{Method} & \multirow{2}{*}{Status} & \multicolumn{2}{c}{Rain} & \multicolumn{2}{c}{Blur} & \multicolumn{2}{c}{Haze} & \multicolumn{2}{c}{Snow} & Avg. PSNR \\
 & & PSNR & SSIM & PSNR & SSIM & PSNR & SSIM & PSNR & SSIM & \\
\midrule
Restormer & Retrained & -- & -- & -- & -- & -- & -- & -- & -- & -- \\
PromptIR & Retrained/Official-ckpt & -- & -- & -- & -- & -- & -- & -- & -- & -- \\
...
\begin{table*}[t]
\centering
\caption{Single-task restoration results. Each method is trained separately for the target task unless marked otherwise. These results support task-specific effectiveness but are not used as the primary evidence for unified restoration.}
